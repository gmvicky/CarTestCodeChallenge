...
protocol CountryCodePickerPresenter: CountryCodePickerDelegate {
    var picker: CountryCodePickerViewController? { get set }
    var onCountrySelected: Action<CountryCodePickerViewController.Country, Void> { get }
    var shouldShowPhone: Bool { get }
    var countryFilter: [String]? { get }
}

extension CountryCodePickerPresenter {
    
    var shouldShowPhone: Bool { return true }
    var countryFilter: [String]? { return nil }
    
    func onCountryCodePickerViewControllerDidPickCountry(_ country: CountryCodePickerViewController.Country) {
        onCountrySelected.execute(country)
        picker?.navigationController?.popViewController(animated: true)
    }
    
    func onShowCountryPicker(rootNavigationController: UINavigationController?, filterd : [String]? = nil) {
        guard let rootNavigationController = rootNavigationController else { return }
        let vc = CountryNumberPickerViewController(phoneNumberKit: PhoneNumberKit())
        vc.delegate = self
        vc.shouldShowPhone = shouldShowPhone
        
        if let countryFilter = countryFilter {
            vc.allCountries =  vc.allCountries.filter { pickerCountry in
                return countryFilter.contains(where: { $0.trimmed == pickerCountry.name })
            }
        }
        
        picker = vc
        let transition = PushTransition()
        transition.viewController = rootNavigationController.viewControllers.first
        vc.presentationTransition = transition
        transition.open(vc)
...
@available(iOS 11.0, *)
open class CountryCodePickerViewController: UITableViewController {
    ...
    lazy var countries: [[Country]] = {
        ...
        let popular = commonCountryCodes.compactMap({ Country(for: $0, with: phoneNumberKit) })
            .filter { aPopular in allCountries.contains(where: { $0.code == aPopular.code }) }
        ...
        if hasCurrent, let current = Country(for: PhoneNumberKit.defaultRegionCode(), with: phoneNumberKit) {
            if allCountries.contains(where: { $0.code == current.code }) {
                hasCurrent = true
    ...
    public override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let prefix = shouldShowPhone ? country.prefix + " " : ""
        cell.textLabel?.text = prefix + country.flag
